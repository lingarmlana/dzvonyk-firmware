...
inline String getIndexPage(String ssid, String ip, String rssi) {
  String page = FPSTR(header_html);
  
  String content = FPSTR(index_content);
  content.replace("%SSID%", ssid);
  content.replace("%IP%", ip);
  content.replace("%RSSI%", rssi);
  
  page += content;
  page += FPSTR(footer_html);
  return page;
}
